 ...
 */
var ADDON_TITLE = 'EMail Notification';
...
function matchVariablesAndReplace(response,line) { 
     
    var prefixPosition = 0, remainingPosition = 0;
    var itemResponses = response.getItemResponses();  
    var mySubstring = line;
    var result = "";
    while( (prefixPosition = mySubstring.indexOf("__##")) != -1) {
      if( prefixPosition != 0 ) {
        result = result + mySubstring.substring(0,prefixPosition);
        mySubstring = mySubstring.substr(prefixPosition);
      }
      
      if( (remainingPosition = mySubstring.indexOf("##__")) != -1) {
         var myVariable = mySubstring.substring("__##".length , remainingPosition );
         var foundItem = null;
         for(var j = 0; j < itemResponses.length; ++j) {
           if(myVariable == itemResponses[j].getItem().getTitle()) {
             foundItem = itemResponses[j].getResponse();
             break;
           }
        }
        if(foundItem != null) {
          ...
          result = result + "__##" + myVariable + "##__";          
        }
        mySubstring = mySubstring.substr(remainingPosition + "##__".length +1);
      } else {
        result = result + mySubstring;
        mySubstring = null;
        break;
      }
    }
    
    if(mySubstring != "" && mySubstring != null) {
      result = result + mySubstring;
    }          
    return result;
...
function sendRespondentNotification(response) {
  ...
  if (respondentEmail) {
    ...
    var lines = settings.getProperty('responseText').split('\n');
    
    for(var i = 0; i < lines.length; i++) {
      lines[i] = matchVariablesAndReplace(response, lines[i]);
    }
    
    template.paragraphs = lines;
    template.notice = NOTICE;
    var message = template.evaluate();
